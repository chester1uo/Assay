...
**引擎:** `assay.engine`(numpy `(T×N)` 后端) · **目录:** `assay.factors.alpha101`  
**面板:** 合成,420 个交易日 × 30 只标的,连续动态,固定随机种子  
**分组:** 为 `indneutralize` alpha 提供 sector/industry/subindustry 标签
...
| Alpha 解析 → 统一 AST | **101 / 101** |
| Alpha 求值(无错误,形状 420×30) | **101 / 101** |
| 对抗式保真度审计(202 个 agent) | **101 / 101 忠实** |
| 在合成面板上产生有限值 | **99 / 101** |
| 在合成面板上全为 NaN(深层链条;在真实数据上可解出) | [96, 97] |
| `indneutralize` alpha(需要 group_data) | 18: [48, 58, 59, 63, 67, 69, 70, 76, 79, 80, 82, 87, 89, 90, 91, 93, 97, 100] |
| 平均覆盖率(有限占比,有限的 alpha) | 82.1% |

## 101 个 alpha 中的算子使用情况
...
*coverage = 有限的 (date,symbol) 单元格占比;warm-up row = 第一个
带有任意有限值的交易日索引(对长窗口/深嵌套的 alpha 较大)。*
