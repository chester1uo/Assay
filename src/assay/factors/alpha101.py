"""The 101 Formulaic Alphas (Kakushadze, 2016; arXiv:1601.00991).

Each alpha is stored as an Assay factor expression. The expressions are the
paper's formulas essentially verbatim — Assay's function-call parser accepts the
WorldQuant / Alpha-101 dialect directly (``rank``, ``delay``, ``correlation``,
``decay_linear``, ``ts_rank``, ``Ts_ArgMax``, ``signedpower``, ``indneutralize``,
the ``adv{{d}}`` macro, the ``? :`` ternary, ``^`` power, ``||`` logical-or, and
``IndClass.sector``/``industry``/``subindustry``). See
``assay_operator_compat.md`` for the full operator mapping.

Evaluate one with :class:`assay.engine.FactorEngine`::

    from assay.factors.alpha101 import ALPHA_101
    from assay.engine import FactorEngine
    eng = FactorEngine(panel, group_data={{"sector": ...}})   # groups needed for indneutralize alphas
    report = eng.evaluate(ALPHA_101[58])

Notes on fidelity to the paper:

* **Windows.** The paper converts a non-integer number of days ``d`` to
  ``floor(d)``; Assay's parser floors fractional windows, so the verbatim
  decimals (e.g. ``decay_linear(x, 7.89291)`` -> window 7) match the paper.
* **adv{{d}}.** The paper defines ``adv{{d}}`` as average daily *dollar* volume;
  Assay maps it to ``ts_mean(volume, d)`` (average *share* volume), per the
  project's operator-compat convention. Substitute a dollar-volume field if
  exact dollar-volume semantics are required.
* **indneutralize** alphas (e.g. #48, #58, #63, #100) demean within an industry
  group and require ``group_data`` (sector/industry/subindustry labels) on the
  engine; without it the engine raises a clear error.
* **signedpower** is sign-preserving (``sign(x) * abs(x)**a``); the paper writes
  it ``x^a`` but names it *signed* power. The plain ``^`` operator maps to
  ``pow`` (no sign preservation).

This module is generated from the paper's Appendix A; do not hand-edit entries.
"""

from __future__ import annotations

# alpha number -> Assay factor expression (paper Appendix A, verbatim)
ALPHA_101: dict[int, str] = {
...
def get(n: int) -> str:
    """Return the Assay expression for alpha number ``n`` (1-101)."""
    try:
        return ALPHA_101[n]
    except KeyError:
        raise KeyError(f"alpha number must be 1-101, got {n}") from None


def all_exprs() -> dict[int, str]:
    """Return a copy of the full ``{number: expression}`` mapping."""
    return dict(ALPHA_101)


# Industry-neutralized alphas: these reference indneutralize(...) and therefore
# require group_data (sector/industry/subindustry) on the FactorEngine.
INDNEUTRALIZE_ALPHAS: tuple[int, ...] = tuple(
    n for n, e in ALPHA_101.items() if "ndneutralize" in e or "ndNeutralize" in e
)
